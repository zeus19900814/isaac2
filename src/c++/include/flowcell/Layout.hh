...
public:
    ...
    unsigned getDataLength() const {return dataCycles_.size();}
    ...
    unsigned getReadNameLength() const {return readNameLength_;}
...
private:
    ...
    /// Maximum length of the fragment name. 0 for bcl flowcels. If fastq or bam name preservation is enabled,
    /// contains the maximum length of the fragment name to be read from the input file, kept in temporary files and
    /// then stored in the result file.
...
inline unsigned getMinReadLength(const flowcell::FlowcellLayoutList &flowcellLayoutList)
{
    unsigned ret = std::numeric_limits<unsigned>::max();
    BOOST_FOREACH(const flowcell::Layout &flowcell, flowcellLayoutList)
    {
        ret = std::min(ret, getMaxReadLength(flowcell.getReadMetadataList()));
    }
    return ret;
}
...
inline unsigned getMaxClusterName(const FlowcellLayoutList &flowcellLayoutList)
{
    return std::max_element(
        flowcellLayoutList.begin(), flowcellLayoutList.end(), boost::bind(&Layout::getReadNameLength, _1))->getReadNameLength();
}
