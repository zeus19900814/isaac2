    ...
    FoundMatchesMetadata &foundMatches)
{
    ...
    BOOST_FOREACH(std::size_t index, seedIndexList)
    {
        ...
        switch (flowcell.getFormat())
        {
            case flowcell::Layout::Bam:
            {
                BamSeedSource<KmerT> dataSource(
                    ...
                    std::min(inputLoadersMax_, coresMax_), seedBaseQualityMin_,
            ...
            case flowcell::Layout::Fastq:
            {
                FastqSeedSource<KmerT> dataSource(
                    ...
                    // the loading itself occurs on one or two threads at a time. So, the real limit is to avoid using
                    // more cores for decompression than the system actually has.
                    // On the other hand, there might be a need to limit the io to 1 thread, while allowing
                    // for the multithreaded processing of other cpu-demanding things.
            ...
            case flowcell::Layout::Bcl:
            {
                BclSeedSource<KmerT> dataSource(
                    ignoreMissingBcls_,
                    inputLoadersMax_, coresMax_, seedBaseQualityMin_, barcodeMetadataList_,
...
template void FindMatchesTransition::perform<oligo::BasicKmerType<28> >(FoundMatchesMetadata &foundMatches);
template void FindMatchesTransition::perform<oligo::BasicKmerType<30> >(FoundMatchesMetadata &foundMatches);
template void FindMatchesTransition::perform<oligo::KmerType>(FoundMatchesMetadata &foundMatches);
template void FindMatchesTransition::perform<oligo::BasicKmerType<34> >(FoundMatchesMetadata &foundMatches);
template void FindMatchesTransition::perform<oligo::BasicKmerType<36> >(FoundMatchesMetadata &foundMatches);
