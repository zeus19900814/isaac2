...
FastqSeedSource<KmerT>::FastqSeedSource(
    ...
    const std::string &qualityEncodingString) :
        ...
        // Notice: with seed generator cluster names are not expected to be present in bcl data.
        // However we need cluster name length to estimate the memory requirements
        clustersAtATimeMax_(clustersAtATimeMax ? clustersAtATimeMax : determineMemoryCapacity(availableMemory, tileClustersMax_, fastqFlowcellLayout.getSeedMetadataList().size(), clusterLength_ + fastqFlowcellLayout.getReadNameLength())),
...
flowcell::TileMetadataList FastqSeedSource<KmerT>::discoverTiles()
{
    ...
    while (!clustersLoaded)
    {
        ...
        clustersLoaded = fastqLoader_.loadClusters(clustersToLoad, 0, fastqFlowcellLayout_.getReadMetadataList(), clustersEnd);
        ...
        alignment::BclClusters &bclData)
{
    ...
    unsigned clustersLoaded = fastqLoader_.loadClusters(
        bclData.getClusterCount(), flowcell.getReadNameLength(), flowcell.getReadMetadataList(), clustersEnd);
...
template class FastqSeedSource<oligo::BasicKmerType<30> >;
template class FastqSeedSource<oligo::BasicKmerType<28> >;
template class FastqSeedSource<oligo::KmerType>;
template class FastqSeedSource<oligo::BasicKmerType<34> >;
template class FastqSeedSource<oligo::BasicKmerType<36> >;
